import styled from 'styled-components'

export const vp_sizes = `
    mobile: 376,
    tablet: 768,
    desktop: 992,
    2k: 1439 
`

export const ar_blue = '#0064B3';
export const ar_darkBlue = '#131E26';
export const ar_darkGrey = '#262526';
export const ar_white = '#FAFAFA';
export const ar_grey = '#BFBFBF';

export const primary_font = '"Montserrat", sans-serif';


export const main_width = '80%';
export const sub_width = '75%';

export const nav_width = '85%';

export const h1_size = '3.5rem';
export const h2_size = '2.5rem';
export const h3_size = '2rem';
export const h4_size = '1.5rem';
export const h5_size = '1.25rem';
export const h6_size = '1rem';

export const p_size = '1rem';

// responsive variables

// $xs_width: 100%; 
// $sm_width: 90%; 
// $md_width: 80%; 
// $lg_width: 75%; 
// $xl_width: 65%; 